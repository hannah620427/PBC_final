        ...
        if t.subtasks:
            ...
            for st in pending:
                ...
                if pct >= 99.0:
                    ...
                    st.completed = True
                    print(f"    ✓ '{st.name}' marked complete.")
                else:
                    st_rem = max(0.0, st.estimated_minutes - done_mins)
                    print(f"    ↓ '{st.name}' {pct:.0f}% done — {st_rem:.0f}m left on this sub-task.")

            # 若本 block 沒有任何子任務被勾選，直接扣 block 時間
            if not any_checked:
