        ...
        for sl in self._block.task_slices:
            ...
            for st in t.subtasks:
                ...
                st_rem_lbl = body_label(st_row,
                                        f"({st_est:.0f}m / {st_est:.0f}m)",
                                        color=T2, size=11)
                st_rem_lbl.pack(side="right")

                st_pct_var = tk.DoubleVar(value=0.0)
                ...
                def _make_check_cmd(st_id, sl_fr, st_rw):
                    def on_toggle():
                        if self._sub_sliders[st_id][0].get():
                            ...
                            sl_fr.pack_forget()
                            # 取消勾選 → slider 歸零，重推主任務與標籤
                            self._sub_sliders[st_id][1].set(0.0)
                ...
                st_pct_lbl = body_label(sl_inner, "0%", color=T1, size=11)
        ...
        for sl in self._block.task_slices:
            ...
            if has_checked_subs:
                ...
                for st_id, data in self._sub_sliders.items():
                    ...
                    # 子任務剩餘 = estimated - done
                    st_rem = max(0.0, est_mins - done_mins)
                    # 若子任務完成（slider ≥ 99%），標記完成
                    if pct_var.get() >= 99.0:
                        st = next((s for s in t.subtasks if s.id == st_id), None)
                        if st:
